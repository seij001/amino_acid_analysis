...
def parse_stride_file(stride_file):
    ...
    with open(stride_file, 'r') as file:
        for line in file:
            ...
            if line.startswith("LOC") and "Strand" in line:
                parts = line.split()
                if len(parts) >= 9:
                    start_res = (parts[2], parts[3], parts[4])  # (Residue type, number, chain)
                    end_res = (parts[6], parts[7], parts[8])    # (Residue type, number, chain)
                    beta_sheets.append((start_res, end_res))
    return beta_sheets
...
def extract_beta_sequences(pdb_residues, beta_sheets):
    ...
    for start_res, end_res in beta_sheets:
        ...
        sequence = [
            ...
            if chain_id == start_chain and start_num <= res_num <= end_num
        ]
...
if __name__ == "__main__":
    pdb_gz_path = "UP000002311_559292_YEAST_v4/AF-A0A0B7P3V8-F1-model_v4.pdb.gz"
